...
const queryPayments = async (req, res) => {
    try {
        ...
        const { startDate, endDate, pageSize = 20, currency, minAmount, maxAmount, last4Digits, entityReferences, receivedEvents } = req.query;
        ...
        if (minAmount)
            queryString += `&minAmount=${minAmount}`;
        if (maxAmount)
            queryString += `&maxAmount=${maxAmount}`;
        if (last4Digits)
            queryString += `&last4Digits=${last4Digits}`;
        if (entityReferences)
            queryString += `&entityReferences=${entityReferences}`;
        if (receivedEvents)
            queryString += `&receivedEvents=${receivedEvents}`;
        console.log(`Querying payments with parameters: ${queryString}`);
        try {
            // Call Worldpay API to query payments
            const response = await (0, node_fetch_1.default)(`https://try.access.worldpay.com/paymentQueries/payments?${queryString}`, {
                method: 'GET',
                headers: {
